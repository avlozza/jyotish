...
with st.expander("Birth Details"):
        ...
        inputdata = jsm.input_birthdata(name=name, gender=gender)

        # Providing Date of birth details
        inputdata = jsm.input_birthdata(year=dob.year, month=dob.month, day=dob.day)

        # Providing Place of birth details
        inputdata = jsm.input_birthdata(place=pob, longitude=long, lattitude=lat, timezone=tz)

        # Providing Time of birth details
        inputdata = jsm.input_birthdata(hour=tob.hour, min=tob.minute, sec=tob.second)
...
if jsm.IsBirthdataValid():
    ...
    if astrodata:
        selected_chart = st.selectbox("Select Chart:", list(astrodata.keys()))
        ...
        if selected_chart in ["D1", "D2", "D3", "D4", "D9", "D10", ...]:  # Add other charts as needed
